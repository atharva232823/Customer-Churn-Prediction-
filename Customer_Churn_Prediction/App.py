...
# -----------------------------
# Load dataset for slider values (UI only)
# -----------------------------
df_data = pd.read_csv(r"C:\Users\Ūśēr̥\Downloads\Telco-Customer-Churn.csv")

# Fix TotalCharges column (convert empty strings to NaN)
...
with left_col:
    ...
    # ============================
    # FIXED: USE DATASET VALUES
    # ============================
    monthly_values = sorted(df_data["MonthlyCharges"].dropna().astype(int).unique())
    total_values = sorted(df_data["TotalCharges"].dropna().astype(int).unique())

    c1, c2 = st.columns(2)
    with c1:
        tenure = st.slider("Tenure (Months)", 0, 100, 12)
        MonthlyCharges = st.select_slider("Monthly Charges", options=monthly_values)
    with c2:
        TotalCharges = st.select_slider("Total Charges", options=total_values)
